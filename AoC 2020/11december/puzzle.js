...
const FindNearestSeatLeft = (array, index, position, count) => {
    if (array[index][position - count] === ".") {
        FindNearestSeatLeft(array, index, position, (count +=1))
    }if (array[index][position - count] === "#") {
        return true
    }  else {
        return false
...
const FindNearestSeatRight = (array, index, position, count) => {
    if (array[index][position + count] === "#") {
        return true
    } if (array[index][position + count] === ".") {
        FindNearestSeatRight(array, index, position, (count +=1))
    } else {
        return false
    }
}
const FindNearestSeatUnder = (array, index, position, count) => {
    if ((index + count) < 99) {
        if (array[index + count][position] === "#") {
            return true
        } if (array[index + count][position] === ".") {
            FindNearestSeatUnder(array, index, position, (count +=1))
        } else {
            return false
        }
    }
}
const FindNearestSeatAbove = (array, index, position, count) => {
    if ((index - count) >= 0) {
        if (array[index - count][position] === "#") {
            return true
        } if (array[index - count][position] === ".") {
            FindNearestSeatAbove(array, index, position, (count +=1))
        } else {
            return false
        }
    }
}
const FindNearestSeatLB = (array, index, position, count) => {
    if (index - count >= 0) {
        if (array[index - count][position - count] === "#") {
            return true
        } if (array[index - count][position - count] === ".") {
            FindNearestSeatLB(array, index, position, (count +=1))
        } else {
            return false
        }
    }
}
const FindNearestSeatRB = (array, index, position, count) => {
    if ((index - count) >= 0) {
        if (array[index - count][position + count] === "#") {
            return true
        } if (array[index - count][position + count] === ".") {
            FindNearestSeatRB(array, index, position, (count +=1))
        } else{
            return false
        }
    }
}
const FindNearestSeatLO = (array, index, position, count) => {
    if ((index + count) < 99) {
        if (array[index + count][position - count] === "#") {
            return true
        } if (array[index + count][position - count] === ".") {
            FindNearestSeatLO(array, index, position, (count +=1))
        } else{
            return false
        }
    }
}
const FindNearestSeatRO = (array, index, position, count) => {
    if ((index + count) < 99) {
        if (array[index + count][position + count] === "#") {
            return true
        } if (array[index + count][position + count] === ".") {
            FindNearestSeatRO(array, index, position, (count +=1))
        } else {
            return false
        }
    }
...
const countOccupiedAdjacentSeats2 = (array, index, position) => {
    occupiedSeats = 0
    if (FindNearestSeatLeft(array, index, position, 1)) {
        occupiedSeats += 1
    }
    if (FindNearestSeatRight(array, index, position, 1)) {
        occupiedSeats += 1
    }
    if (FindNearestSeatAbove(array, index, position, 1)) {
        occupiedSeats += 1
    }
    if (FindNearestSeatUnder(array, index, position, 1)) {
        occupiedSeats += 1
    }
    if (FindNearestSeatLO(array, index, position, 1)) {
        occupiedSeats += 1
    }
    if (FindNearestSeatLB(array, index, position, 1)) {
        occupiedSeats += 1
    }
    if (FindNearestSeatRO(array, index, position, 1)) {
        occupiedSeats += 1
    }
    if (FindNearestSeatRB(array, index, position, 1)) {
        occupiedSeats += 1
    }
    return occupiedSeats
}
...
const applyRules2 = array => array.map((item, index) => item.map((item, position) => {
    if (item === "L" && (countOccupiedAdjacentSeats2(array, index, position) === 0)) {
        return item.replace("L", "#")
    }
    if (item === "#" && (countOccupiedAdjacentSeats2(array, index, position) >= 5)) {
        return item.replace("#", "L")
    }
    else {
        return item
...
// const firstRound = applyRules2(seatArrays)
...
const solution2 = array => {
    arrayCount2 ++
    let newArray = applyRules2(array);
    if (JSON.stringify(newArray) === JSON.stringify(array)) {
        console.log(arrayCount2)
        console.log(newArray.flat().filter(item => item === "#").length)
        return newArray
    } else {
        solution2(newArray)
...
// console.log(seatArrays.length)

console.log(solution2(seatArrays))
