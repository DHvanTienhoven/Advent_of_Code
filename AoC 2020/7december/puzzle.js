...
solution(containsGold);

//part 2

const findBagRules = bag => splittedRules.filter(item => item[0].includes(bag)).flat()[1].split(",");

const goldBagRules = findBagRules("shiny gold bag");
const blueBagRules = findBagRules("drab blue bag")
const salmonBagRules = findBagRules("dull salmon bag")

console.log(goldBagRules)
// console.log(blueBagRules)
// console.log(salmonBagRules)

const secondaryRules = goldBagRules.map(rule => {
    if (typeOf.parseInt(rule.slice(1, 2)) === "number") {
        const newRules = findBagRules(rule.slice(3, -3))
        newRules.map(newRule)
        parseInt(rule.slice(1, 2))
    }
})

console.log(secondaryRules)